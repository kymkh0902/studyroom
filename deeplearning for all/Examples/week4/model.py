...
class Model(object):
    ...
    def build_neural_net(self):
        ...
        fc2 = tf.layers.dense(fc1, units=self.num_unit2, activation=tf.nn.relu,
                              bias_initializer=tf.zeros_initializer(),
                              kernel_initializer=tf.contrib.layers.xavier_initializer())
        fc2 = tf.nn.dropout(fc1, keep_prob=self.keep_prob)
        fc3 = tf.layers.dense(fc2, units=self.num_unit3, activation=tf.nn.relu,
                              bias_initializer=tf.zeros_initializer(),
                              kernel_initializer=tf.contrib.layers.xavier_initializer())
        fc3 = tf.nn.dropout(fc1, keep_prob=self.keep_prob)
        fc4 = tf.layers.dense(fc3, units=self.num_unit4, activation=tf.nn.relu,
                              bias_initializer=tf.zeros_initializer(),
                              kernel_initializer=tf.contrib.layers.xavier_initializer())
        fc4 = tf.nn.dropout(fc1, keep_prob=self.keep_prob)
    ...
    ## 모델을 eval 하기
    def eval(self, sess, X, Y, keep_prob):
        input_feed = {}
        input_feed[self.X] = X
        input_feed[self.Y] = Y
        input_feed[self.keep_prob] = keep_prob
